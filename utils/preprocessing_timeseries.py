...
def create_chunks_from_dataset(dataset, chunk_size=2000, overlap=0.5):
    """
    Splits each EEG sample into overlapping chunks.
    ...
    Returns:
        List[FrequencyTokensData]: List of chunked samples.
    """
    chunked_samples = []
...
def custom_collate_fn(batch):
    """
    Custom collate function to stack FrequencyTokensData batches.

    Args:
        batch (List[FrequencyTokensData]): A batch of samples.

    Returns:
        Dict[str, Union[Tensor, List[str]]]: Batched tensors and IDs.
    """
...
def create_training_data(root, chunk_size=2000, overlap=0.5, batch_size=16):
    """
    Loads EEG dataset, creates chunks, and returns data loaders.
...
def evaluate(model, dataloader, device, num_classes):
    """
    Evaluate model on a given validation set.
    ...
    Returns:
        Tuple[float, float, float, float]: Validation loss, BAC, AUC-PR, and AUROC.
    """
    model.eval()
...
def main():
    """
    Main training loop for the XAIguiFormerTimeSeries model.
    """
    ...
    print("\nTraining finished. Best validation BAC: {:.4f} at epoch {}".format(best_val_bac, best_epoch))
    print("\nSummary of metrics over {} epochs".format(epochs))
